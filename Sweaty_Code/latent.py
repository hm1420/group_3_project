...
class tictactoe:  # Main Class
    ...
    def strategy2(self):
        ...
        #checking rows
        for i in range(3):
            if last_board[i][0]== last_board[i][1] and last_board[i][2]=='-':
                return i, 2
            else:
                if last_board[i][0]==last_board[i][2] and last_board[i][1]=='-':
                    return i, 1
                else:
                    if last_board[i][1]==last_board[i][2] and last_board[i][0]=='-':
                        return i, 0
                    
        #checking for columns
        for i in range(3):
            if last_board[0][i]== last_board[1][i] and last_board[2][i]=='-':
                return 2,i
            else:
                if last_board[0][i]==last_board[2][i] and last_board[1][i]=='-':
                    return 1, i
                else:
                    if last_board[1][i]==last_board[2][i] and last_board[0][i]=='-':
                        return 0,i
        #checking for diagonals
        if last_board[0][0]==last_board[1][1] and last_board[2][2]=='-':
            return 2,2
        else:
            if last_board[0][0]==last_board[2][2] and last_board[1][1]=='-':
                return 1,1
            else:
                if last_board[1][1]==last_board[2][2] and last_board[0][0]=='-':
                    return 0,0
        if last_board[0][2]==last_board[1][1] and last_board[2][0]=='-':
            return 2,0
        else:
            if last_board[0][2]==last_board[0][2] and last_board[1][1]=='-':
                return 1,1
            else:
                if last_board[1][1]==last_board[2][0] and last_board[0][2]=='-':
                    return 0,2
        return False
